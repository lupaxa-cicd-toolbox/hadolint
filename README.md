...
jobs:
  build:
    ...
    steps:
      ...
      - name: Run Hadolint
        run: bash <(curl -s https://raw.githubusercontent.com/lupaxa-cicd-toolbox/hadolint/master/src/pipeline.sh)
```
...
```bash
bash <(curl -s https://raw.githubusercontent.com/lupaxa-cicd-toolbox/hadolint/master/src/pipeline.sh)
```
